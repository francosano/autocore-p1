...
#   powershell -ExecutionPolicy Bypass -File scripts\site-sync-local.ps1          (sync a Supabase)
#   powershell -ExecutionPolicy Bypass -File scripts\site-sync-local.ps1 -Dry     (probar sin escribir)
#   powershell -ExecutionPolicy Bypass -File scripts\site-sync-local.ps1 -Fotos   (bajar TODAS las fotos a .\fotos\)
param([switch]$Dry, [switch]$Fotos)

$ErrorActionPreference = 'Stop'
$repo = Split-Path -Parent $PSScriptRoot

if (-not $Dry -and -not $Fotos) {
...
try {
    Push-Location $repo
    if ($Fotos) { node scripts/site-sync-local.mjs --fotos }
    elseif ($Dry) { node scripts/site-sync-local.mjs --dry }
    else { node scripts/site-sync-local.mjs }
...
if ($Fotos) { Start-Process explorer.exe (Join-Path $repo 'fotos') }
